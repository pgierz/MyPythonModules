...
def plot_var_anom_from_ncdf_file_timestep(varname, ts, file, cfile, mm, ovarname=None, **cfopts):
    ...
    mm.drawmapboundary(fill_color='gray')
    lons, lats = np.meshgrid(lon, lat)
    cf = mm.contourf(lons, lats, var, latlon=True, extend="both",
                     **cfopts)
    return cf
...
def plot_overturning_contours(fin, ax, labels=True, **cfopts):
    dat = fin.variables["var101"].data
    if hasattr(fin.variables["var101"], 'missing_value'):
        # Mask out missing values based on what netcdf thinks the missing value is!
        dat = np.ma.masked_equal(dat, fin.variables["var101"].missing_value)
...
def plot_overturning_anom_contours(fin, ctrl, ax, labels=True, **cfopts):
    rdat = fin.variables["var101"].data
    cdat = ctrl.variables["var101"].data
    if hasattr(fin.variables["var101"], 'missing_value'):
        # Mask out missing values based on what netcdf thinks the missing value is!
