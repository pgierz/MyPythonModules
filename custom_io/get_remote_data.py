...
def _refile_local_file_correctly(filepath, where_to_go):
    pass
...
def _copy_remote_file(filepath, remote_dump="/tmp/remote_data/"):
    ...
    if not os.path.exists(remote_dump + os.path.basename(filepath)):
        for r in [line.strip() for line in open(os.path.expanduser("~/.all_runs.dat")) if "#" not in line]:
            test = _check_for_local(os.path.basename(filepath))
            if test:
                break
        if not test:
            ...
            _refile_local_file_correctly(filepath, "where_to_go")
...
def get_remote_data(filepath, time=False, info=False, copy_to_local=False):
    ...
    if time:
        print pre_s.ljust(40)
            +
                print_colors.OKGREEN("{filepath}").format(
                    filepath=os.path.basename(filepath)).ljust(100)
            + " in ".rjust(0)
            +
                print_colors.OKBLUE("{time}").format(
                    time=round(time.time() - now))
            + " seconds"
    if info:
